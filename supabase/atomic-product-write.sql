...
begin
  ...
  if p_product_id is null then
    insert into public.products (
      store_id, name, description, price, image_path, is_available, is_visible
    ) values (
      p_store_id, trim(p_name), trim(p_description), p_price, v_primary_path, p_is_available, p_is_visible
...
revoke all on function public.save_product_with_gallery(uuid, uuid, text, text, numeric, boolean, boolean, jsonb) from public;
grant execute on function public.save_product_with_gallery(uuid, uuid, text, text, numeric, boolean, boolean, jsonb) to authenticated, service_role;
