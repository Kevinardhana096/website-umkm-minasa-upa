...
export function StoreProfileForm({ store, isSaving, onSave }: StoreProfileFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-gray-100 bg-white p-6 sm:p-8 shadow-xs">
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-gray-100 bg-gray-50/70 p-3.5"><input type="checkbox" checked={form.isActive} onChange={(event) => setForm((current) => ({ ...current, isActive: event.target.checked }))} className="mt-0.5 h-4 w-4 rounded border-gray-300 text-[#0F2C23] focus:ring-[#0F2C23]" /><span><span className="block text-sm font-bold text-gray-800">Tampilkan toko di katalog publik</span><span className="mt-0.5 block text-xs text-gray-500">Matikan sementara jika toko sedang tidak menerima pesanan.</span></span></label>

      <div className="flex justify-end border-t border-gray-100 pt-4"><button type="submit" disabled={isSaving} className="inline-flex items-center gap-2 rounded-xl bg-[#0F2C23] px-5 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-[#184537] disabled:cursor-not-allowed disabled:opacity-60">{isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{isSaving ? "Menyimpan..." : "Simpan perubahan"}</button></div>
    </form>
  );
}
