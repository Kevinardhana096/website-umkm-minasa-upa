...
11c. Jalankan `supabase/max-featured-products.sql` untuk membatasi produk unggulan menjadi maksimal empat.
...
Konfigurasi lama `AI_CHAT_API_URL`, `AI_CHAT_API_KEY`, dan `AI_CHAT_MODEL` tetap diterima sebagai konfigurasi Gemini utama. Semua API key tidak boleh menggunakan awalan `NEXT_PUBLIC_` dan tidak pernah dikirim ke browser. Knowledge profil dipakai untuk pertanyaan statis; metadata provenance dikirim bersama jawaban knowledge dan dimasukkan ke context provider agar status `draft` tidak dipresentasikan sebagai data resmi. Pertanyaan terbaru memakai native Gemini `generateContent` dengan `google_search`; jika gagal, sistem mencoba Mistral Conversations API dengan `web_search`. Keduanya memiliki timeout 12 detik, menyimpan maksimal 5 sumber, dan menampilkan sumber web yang lolos validasi pada jawaban chat. Endpoint membatasi pertanyaan maksimal 800 karakter, sekitar 20 request per alamat IP per menit, dan waktu tunggu provider 8 detik. Jika `UPSTASH_REDIS_REST_URL` dan `UPSTASH_REDIS_REST_TOKEN` tersedia, rate limit memakai Upstash Redis terdistribusi; jika belum tersedia atau gagal dihubungi, limiter lokal berbatas tetap digunakan sebagai pengaman.
